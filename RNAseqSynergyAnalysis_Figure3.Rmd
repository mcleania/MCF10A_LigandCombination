...
identify_synergy<-function(Combo_DE_File1, Combo_DE_File2, condition1 = 'EGF', condition2 = 'OSM',combocondition = 'EGF_OSM'){
  ...
  rename(!!comparison1:=hgnc_symbol, log2FoldChange, !!comparison1_pval:=padj) %>%
  ...
  rename(!!comparison2:=hgnc_symbol, log2FoldChange, !!comparison2_pval:=padj) %>%
  ...
  combo_1_dge_15_up<- combo_dge_15lfc %>%
    filter(!!comparison1 > 1.5 & !!comparison1_pval < .05) 

  combo_1_dge_15_down<-combo_dge_15lfc %>%
    filter(!!comparison1 < (-1.5) & !!comparison1_pval < .05) 

  combo_2_dge_15_up<-combo_dge_15lfc %>%
    filter(!!comparison2 > 1.5 & !!comparison2_pval < .05) 
  ...
  combo_label<-combo_dge %>%
    mutate(label = ifelse(ensembl_gene_id %in% combo_1_dge_15_up$ensembl_gene_id & ensembl_gene_id %in% test = combo_2_dge_15_up$ensembl_gene_id, 'Positively Synergistic',ifelse(ensembl_gene_id %in% combo_1_dge_1_down$ensembl_gene_id & ensembl_gene_id %in% combo_2_dge_1_down$ensembl_gene_id, 'Negatively Synergistic', 'n')))
...
egf_egfosm_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_EGF_OSM_vs_EGF.csv')
osm_egfosm_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_EGF_OSM_vs_OSM.csv')
tgfb_egftgfb_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_EGF_TGFB_vs_EGF.csv')
egf_egftgfb_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_EGF_TGFB_vs_TGFB.csv')
tgfb_osmtgfb_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_OSM_TGFB_vs_OSM.csv')
osm_osmtgfb_dge<-read_csv('MDD_ligandCombination_RNAseq_DETable_allGenes_OSM_TGFB_vs_OSM.csv')
```
...
ggplot(combo_label_eo, aes(x=Combo_vs_EGF_LFC, y =Combo_vs_OSM_LFC, color = label)) +
